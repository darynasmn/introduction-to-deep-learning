...
def batchnorm_backward(dout, cache):
    ...
    _, x_norm, _, gamma, x_minus_mean, ivar, sqrtvar, var, eps = cache
    ...
    dminus_mean = -1 * np.sum(dx_minus_mean1+dx_minus_mean2, axis=0)

    dx2 = 1. /N * np.ones((N,D)) * dminus_mean
...
def batchnorm_backward_alt(dout, cache):
    ...
    dxhat = dout * gamma
    dx = (1.0 / N) * ivar * (N * dxhat - np.sum(dxhat, axis=0) - x_norm * np.sum(dxhat * x_norm, axis=0))
